...
class OnUpdateValidator extends BaseValidator
{
	protected static $validActions = array(
		JoinInterface::ACTION_ASSOCIATE,
		JoinInterface::ACTION_UPDATE,
	...
	public function validate($join, array $options = array())
	{
		...
		$value = null;
		if (is_object($join) && property_exists($join, 'onUpdate')) {
			$value = $join->onUpdate;
		}

		if (!is_string($value) || !in_array($value, self::$validActions)) {
			$error = $this->buildError('Join onUpdate value must be one of the following: ' . implode(', ', self::$validActions));
			$errors->push($error);
		}
