...
trait ResourceBuilderAssertions
{
	public function assertBuiltTableMatchesUserManifest(Definition\Table $table)
	{
		...
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\FieldList', $table->fields);
		$this->assertEquals(3, $table->fields->length());
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\JoinList', $table->links);
		$this->assertEquals(3, $table->links->length());
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\ValidatorList', $table->validators);
		...
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Field', $table->fields->getByIndex(0));
		$this->assertSame($table, $table->fields->getByIndex(1)->table);
		$this->assertEquals('id', $table->fields->getByIndex(0)->name);
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\ValidatorList', $table->fields->getByIndex(0)->validators);
		...
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Field', $table->fields->getByIndex(1));
		$this->assertSame($table, $table->fields->getByIndex(1)->table);
		$this->assertEquals('forename', $table->fields->getByIndex(1)->name);
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\ValidatorList', $table->fields->getByIndex(1)->validators);
		...
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Field', $table->fields->getByIndex(2));
		$this->assertSame($table, $table->fields->getByIndex(2)->table);
		$this->assertEquals('surname', $table->fields->getByIndex(2)->name);
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\ValidatorList', $table->fields->getByIndex(2)->validators);
	...
	public function assertBuiltUserTableJoinsToFriendsTable(Definition\Table $table)
	{
		...
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\TableList', $linkToFriends->intermediaryTables);
		...
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Join\ConstraintList', $linkToFriends->getConstraints());
		$this->assertEquals(1, $linkToFriends->getConstraints()->length());

		$joinToFriends = $linkToFriends->getConstraints()->getByIndex(0);
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Join\Constraint', $joinToFriends);
		$this->assertSame($linkToFriends, $joinToFriends->link);
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Field', $joinToFriends->parentField);
		$this->assertEquals('id', $joinToFriends->parentField->name);
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Field', $joinToFriends->childField);
		$this->assertEquals('id', $joinToFriends->childField->name);

		$subJoinsToFriends = $linkToFriends->getConstraints()->getByIndex(0)->subJoins;
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Join\SubJoinList', $subJoinsToFriends);
		$this->assertEquals(2, $subJoinsToFriends->length());

		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Join\SubJoin', $subJoinsToFriends->getByIndex(0));
		$this->assertSame($table, $subJoinsToFriends->getByIndex(0)->parentTable);
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Field', $subJoinsToFriends->getByIndex(0)->parentField);
		...
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table', $subJoinsToFriends->getByIndex(0)->childTable);
		...
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Field', $subJoinsToFriends->getByIndex(0)->childField);
		...
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Join\SubJoin', $subJoinsToFriends->getByIndex(1));
		$this->assertSame($subJoinsToFriends->getByIndex(0)->childTable, $subJoinsToFriends->getByIndex(1)->parentTable);

		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Field', $subJoinsToFriends->getByIndex(1)->parentField);
		...
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table', $subJoinsToFriends->getByIndex(1)->childTable);
	...
	public function assertBuiltUserTableJoinsToPostsTable(Definition\Table $table)
	{
		...
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Join', $linkToPosts);
		...
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Join\ConstraintList', $linkToPosts->getConstraints());
		$this->assertEquals(1, $linkToPosts->getConstraints()->length());

		$joinToPosts = $linkToPosts->getConstraints()->getByIndex(0);
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Join\Constraint', $joinToPosts);
		$this->assertSame($linkToPosts, $joinToPosts->link);
		$this->assertNull($joinToPosts->subJoins);

		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Field', $joinToPosts->parentField);
		...
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Field', $joinToPosts->childField);
		$this->assertEquals('authorId', $joinToPosts->childField->name);
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table', $joinToPosts->childField->table);
		...
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\ValidatorList', $joinToPosts->childField->validators);
	...
	public function assertBuiltUserTableJoinsToAddressSubTable(Definition\Table $table)
	{
        ...
        $this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Join', $linkToAddress);
        ...
        $this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Join\ConstraintList', $linkToAddress->getConstraints());
        $this->assertEquals(1, $linkToAddress->getConstraints()->length());

        $joinToAddress = $linkToAddress->getConstraints()->getByIndex(0);
        $this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Join\Constraint', $joinToAddress);
        $this->assertSame($linkToAddress, $joinToAddress->link);
        $this->assertNull($joinToAddress->subJoins);

        $this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Field', $joinToAddress->parentField);
        ...
        $this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Field', $joinToAddress->childField);
        $this->assertEquals('userId', $joinToAddress->childField->name);
        $this->assertInstanceOf('Sloth\Module\Resource\Definition\Table', $joinToAddress->childField->table);
        ...
        $this->assertInstanceOf('Sloth\Module\Resource\Definition\ValidatorList', $joinToAddress->childField->validators);
	...
	public function assertBuiltTableMatchesPostManifest(Definition\Table $table)
	{
		...
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table', $table);
		$this->assertEquals('Post', $table->name);
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\FieldList', $table->fields);
		$this->assertEquals(3, $table->fields->length());
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\JoinList', $table->links);
	...
	public function assertBuiltPostTableJoinsToAuthorTable(Definition\Table $table)
	{
		...
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Join\ConstraintList', $linkToAuthor->getConstraints());
		$this->assertEquals(1, $linkToAuthor->getConstraints()->length());

		$joinToAuthor = $linkToAuthor->getConstraints()->getByIndex(0);
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Join\Constraint', $joinToAuthor);
		$this->assertSame($linkToAuthor, $joinToAuthor->link);
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Field', $joinToAuthor->parentField);
		$this->assertEquals('authorId', $joinToAuthor->parentField->name);
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Field', $joinToAuthor->childField);
		...
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table', $joinToAuthor->childField->table);
		...
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\ValidatorList', $joinToAuthor->childField->validators);
	...
	public function assertBuildPostsTableJoinsToCommentsTable(Definition\Table $table)
	{
		...
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Join', $linkToComments);
		...
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Join\ConstraintList', $linkToComments->getConstraints());
		$this->assertEquals(1, $linkToComments->getConstraints()->length());

		$joinToComments = $linkToComments->getConstraints()->getByIndex(0);
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Join\Constraint', $joinToComments);
		$this->assertSame($linkToComments, $joinToComments->link);
		$this->assertNull($joinToComments->subJoins);

		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Field', $joinToComments->parentField);
		...
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table\Field', $joinToComments->childField);
		$this->assertEquals('postId', $joinToComments->childField->name);
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\Table', $joinToComments->childField->table);
		...
		$this->assertInstanceOf('Sloth\Module\Resource\Definition\ValidatorList', $joinToComments->childField->validators);
